...
from ubot import ubot, get_expired_date, DEVS, get_seles, get_time, Button, unpackInlineMessage, get_var, bot, start_time
...
async def alive_query(client, inline_query):
    ...
    for my in ubot._ubot:
        if int(get_id[2]) == my.me.id:
            ...
            exp = get_exp.strftime("%d-%m-%Y") if get_exp else "Tidak diketahui"

            status = "<b>ɪᴘʜᴏɴᴇ</b> <code>[𝟷𝟵ᴘʀᴏᴍᴀx]</code>" if my.me.id in DEVS else "<b>ɪᴘʜᴏɴᴇ</b> <code>[𝟷𝟻ᴘʀᴏ]</code>" if my.me.id in await get_seles() else "<b>ɪᴘʜᴏɴᴇ</b> <code>[𝟷𝟺ᴘʀᴏᴍᴀx]</code>"
            ...
            antipm = "disable" if not await get_var(my.me.id, "ENABLE_PM_GUARD") else "enable"

...
async def tts_cmd(client, message):
    ...
    if message.reply_to_message:
        if isinstance(client._translate.get(client.me.id), dict):
            language = client._translate[client.me.id].get("negara", "en")  # Default language
        else:
...
async def tr_cmd(client, message):
    ...
    if message.reply_to_message:
        if isinstance(client._translate.get(client.me.id), dict):
            dest = client._translate[client.me.id].get("negara", "en")  # Default language
        else:
            dest = "id"  # Default language
...
async def set_bahasa_callback(client, callback_query):
    ...
    try:
        ...
        if isinstance(m._client._translate, dict):
